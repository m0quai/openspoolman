...
# 18. Änderung 2026-08-20 18:38 – Desktop-mscorlib explizit deaktiviert

## Fehlerbild

Trotz korrektem HintPath auf die nanoFramework-CoreLibrary löste MSBuild zusätzlich die Desktop-.NET-Framework-`mscorlib` aus dem Windows-GAC auf:

```text
C:\Windows\Microsoft.NET\assembly\GAC_64\mscorlib\...
```

Der nanoFramework-`GenerateBinaryOutputTask` versuchte anschließend zu dieser Desktop-Assembly eine `.pe`-Datei zu finden und brach ab.

## Ursache

Ein klassisches C#-MSBuild-Projekt fügt standardmäßig die Plattform-`mscorlib` hinzu. Für nanoFramework ist das falsch, weil die eigene `nanoFramework.CoreLibrary` die Core-Library bereitstellt.

## Korrektur

Im projektspezifischen `.nfproj` werden jetzt explizit gesetzt:
...
`NoStdLib=true` entspricht dem Compilerverhalten `/nostdlib+` und verhindert, dass die Desktop-.NET-Framework-`mscorlib` automatisch hinzugefügt wird.

Die einzige `mscorlib`-Referenz bleibt:

```text
$(SolutionDir)packages\nanoFramework.CoreLibrary.1.17.11\lib\mscorlib.dll
```

`AnyCPU` bleibt unverändert.
...
2. `bin` und `obj` im Projekt `OpenSpoolMan.Esp32` löschen.
3. Visual Studio neu starten.
4. Solution öffnen.
5. `Rebuild Solution`.
6. Es darf keine `mscorlib` unter `C:\Windows\Microsoft.NET\assembly\GAC_64` mehr aufgelöst werden.
